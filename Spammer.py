...
def bann_text():
    clr()
    logo = """
  ____    ____       _      __  __   __  __   _____   ____  
 / ___|  |  _ \     / \    |  \/  | |  \/  | | ____| |  _ \ 
 \___ \  | |_) |   / _ \   | |\/| | | |\/| | |  _|   | |_) |
  ___) | |  __/   / ___ \  | |  | | | |  | | | |___  |  _ < 
...
def do_zip_update():
    ...
    if success:
        mesgdcrt.SuccessMessage("Spammer was updated to the latest version")
        mesgdcrt.GeneralMessage(
            "Please run the script again to load the latest version")
    else:
        mesgdcrt.FailureMessage("Unable to update Spammer.")
        mesgdcrt.WarningMessage(
            "Grab The Latest one From https://github.com/RajTechNation/SMS-SPAMMER.git")
...
def do_git_update():
    ...
    if success:
        ...
        print(
            "git checkout . && "
            "git pull https://github.com/RajTechNation/SMS-SPAMMER.git HEAD")
    sys.exit()
...
def check_for_updates():
    ...
    if fver != __VERSION__:
        ...
        mesgdcrt.SuccessMessage("Spammer is up-to-date")
        mesgdcrt.GeneralMessage("Starting Spammer")
...
def pretty_print(cc, target, success, failed):
    ...
    mesgdcrt.SectionMessage("Spamming is in progress - Please wait")
    mesgdcrt.GeneralMessage(
        "Please stay connected to the internet during Spamming")
    mesgdcrt.GeneralMessage("Target       : " + cc + " " + target)
    mesgdcrt.GeneralMessage("Sent         : " + str(requested))
    mesgdcrt.GeneralMessage("Successful   : " + str(success))
    mesgdcrt.GeneralMessage("Failed       : " + str(failed))
    mesgdcrt.WarningMessage(
        "This tool is made for fun and research purposes only")
    mesgdcrt.SuccessMessage("Spammer was created by RTN")
...
def workernode(mode, cc, target, count, delay, max_threads):
    ...
    clr()
    mesgdcrt.SectionMessage("Gearing up the Spammer - Please wait")
    mesgdcrt.GeneralMessage(
        "Please stay connected to the internet during Spamming")
            ...
            for job in as_completed(jobs):
                ...
                if result is None:
                    mesgdcrt.FailureMessage(
                        "Spamming limit for your target has been reached")
    ...
    print("\n")
    mesgdcrt.SuccessMessage("Spamming completed!")
    time.sleep(1.5)
    bann_text()
    sys.exit()
...
if sys.version_info[0] != 3:
    mesgdcrt.FailureMessage("Spammer will work only in Python v3")
    sys.exit()
...
description = """Spammer - Your Friendly Spammer Application

Spammer can be used for many purposes which incudes -
\t Exposing the vulnerable APIs over Internet
\t Friendly Spamming
\t Testing Your Spam Detector and more ....

Spammer is not intented for malicious uses.
"""

parser = argparse.ArgumentParser(description=description,
                                 epilog='Coded by SpeedX !!!')
parser.add_argument("-sms", "--sms", action="store_true",
                    help="start Spammer with SMS Bomb mode")
parser.add_argument("-call", "--call", action="store_true",
                    help="start Spammer with CALL Bomb mode")
parser.add_argument("-mail", "--mail", action="store_true",
                    help="start Spammer with MAIL Bomb mode")
parser.add_argument("-ascii", "--ascii", action="store_true",
                    help="show only characters of standard ASCII set")
parser.add_argument("-u", "--update", action="store_true",
                    help="update Spammer")
parser.add_argument("-c", "--contributors", action="store_true",
                    help="show current Spammer contributors")
parser.add_argument("-v", "--version", action="store_true",
                    help="show current Spammer version")
